...
missing_by_var_plot<- ggplot(missing_by_var, 
                             aes(x = fct_reorder(variable, pct_miss), y = pct_miss)) + 
  geom_bar(stat = "identity") + coord_flip() + xlab("Variable") + ylab("Percentage Missing") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1L, scale = 1)) 

  missing_by_var_plot<- missing_by_var_plot + aes(text = paste("N Missing: ", n_miss, "<br>",
