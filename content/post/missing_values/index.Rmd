...
Before doing much else, it's often a good idea to take a look at the range of unique values for each variable — this lets us identify odd values that may either be mistakes or missing value codes.
...
As you can see, missing values are now explicit and not represented by heterogeneous digits/letters. 
...
I think it can be helpful to have mental template for exploring missing values on three levels: (1) At the level of the whole dataframe, (2) at the level of rows (i.e. cases), and (3) at the level of individual variables.
...
all_missing_summary<-  all_missing %>% miss_case_summary() %>% mutate(identifier = all_missing$id)
...
Cases that have a missing value on the identifier column are aggregated to make a single bar called NULL. So you'll have to hover along this bar to get the info about the three individual cases that it comprises.

Another helpful way of evaluting row-wise missingness is using a raster or tile plot. One function from the `visdat` package is great for this.
...
After exploring row-wise missingness, it's time to identify missing values on a by-variable basis — this gives important info about the variables that people don't want to report on. First, let's work out the percentage of our variables with missing data.
