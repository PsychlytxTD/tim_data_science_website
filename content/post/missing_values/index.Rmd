...
Before doing much else, it is often a good idea to take a look at the uniqe range of values within each variable — this lets us identify odd values that may either be mistakes or missing value codes.
...
This is where `naniar` becomes really useful. Using this package, we can replace any predetermined missing value codes (e.g. -999) with explicit NA values.
...
Here we can see, for example, that case (row) 6 has missing values for`id` and `sex` but complete values for all the other variables.

##Variable-Wise Missing Data Exploration

After exploring row-wise missingness, it is time to identify missing values on a variable by variable basis. First, we can work out the percentage of our variables that have missing data.
...
It is also easy to break down missing values across variables **within levels** of a grouping variable as follows:
