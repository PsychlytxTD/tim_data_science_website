---
title: Mastering Missing Data Analysis
...
data<- tibble::tribble(
  ...
  "PHQ", "02/02/2003",   1,       1,     uuid::UUIDgenerate(),   -999,
...
As you can tell, is a disaster in terms of consistent coding of missing values in the dataframe. A whole bunch of diffirent values have seemingly been used to denote missingness.
...
Assumeing our file is `called outcomes.csv`:

`data<- readr::read_csv("outcomes.csv", na = c("N/A", ".", "", "999", "-98"))`
...
data_na<- data %>% naniar::replace_with_na_if(.predicate = is.character, condition = ~.x %in% na_vals)
...
all_missing_summary<- miss_case_summary(all_missing) %>% dplyr::mutate(identifier = all_missing$id)
...
Another helpful way of evaluting row-wise missingness is using a raster or tile plot. The `visdat()` function from the visdat package is great for this, but we'll rotate it to have rows on the y-axis, in case there are a lot of them. 
