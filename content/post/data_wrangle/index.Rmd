---
title: "Supercharged Data Cleaning with R (Part Two - Data Wrangling)"
...
Let's use it to pull in that dataframe.
...
It is now time to wrangle my data into a useable form.

First — I need to clean up my variable names. They are a mess of inconsistent upper and lower casing and have letters in strange places. For example, `EEnd_BPIPainInterference`.

The `make_clean_names()` function from `janitor` will fix the majority of problems with variable names
...
You'll see that variable names are now all lower case, and spearated with underscores.

Now I want to get rid of the strange `e_` that preceeds the time 2 variables (e.g. `e_end_bpi_pain_interference`). I also want to put an underscore between any words and digits. Regex simplifies both of these tasks.
...
The next task is to make sure that missing data has been coded consistently. From my knowledge of the data, I am aware that missing responses can be indicated by `-2`, `-5`, `-4` and `99`, depending on the variable. But missing data might appear in other ways too.

The `Naniar` package has a pre-made `common_na_strings` object (a character vector), which lists many commonly used conventions that are commonly used to denote missingness (e.g. "n/a", "N/A", "missing"). I add the values above to this object and then explicitly encode all missing responses in the dataframe as `NA`.
...
I then want to remove any rows in the dataframe that are completely empty, and to remove rows that have the same `patient_id`, since this indicates row duplication.
...
Next, I ensure that quantitative variables have a numeric data type. The easiest way to accomplish this is to first store the names of quantitative variables in a character vector. I then use this vector to transform the variables of interest.
...
At the moment, there are several character variables that have only two possible values — 0 or 1. It would be best to convert their data type to numeric, so that I can include them in modelling analyses later on (e.g. regression).
...
Another task is to convert the codes of some character variables into meaningful categories. Because this step is lengthy, I don't include all the output. A few examples of the recoding process should suffice.
...
Unfortunately, the recoding process has led to a large number of possible categories for some character variables. Do I really need to distinguish whether a person's injury occured to their left knee or right knee? Surely a category like `leg` would be better for any analyses.
...
#Drop miscellaneous variables that I know I don't kneed
